...
public class MovieService implements IMovieService {
	...
	/**
	 * JSOUP 라이브러리를 통한 CGV 영화순위정보 가져오기
	 */
	@Override
	public int getMovieInfoFromWEB() throws Exception {
		...
		// CGV 웹페이지의 전체 소스 중 일부 태크를 선택하기 위하여 사용
		// <div class="sect-movie-chart"> 이 태그 내에 있는 HTML소스만 element에 저장
		Elements element = doc.select("div.sect-movie-chart");

		// Iterator를 사용하여 영화순위 정보를 가져오기
		...
		while (movie_rank.hasNext()) {
			...
			// 영화 순위(trim 함수 추가 이유 : trim함수는 글자의 앞뒤 공백 삭제하여 데이터 수집시, 홈페이지 개발자들을 앞뒤 공백 집어넣을 수
			// 있어서 추가)
			...
			// 수집되는 데이터가 '2019.10.23개봉' 이기 때문에 앞에 10자리만 저장
