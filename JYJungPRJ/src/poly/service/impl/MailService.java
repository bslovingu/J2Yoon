...
public class MailService implements IMailService {
	...
	final String user = "sincethe1997"; // 본인 네이버 아이디
	final String password = "g*89*597*9"; // 본인 네이버 비밀번호
	...
	public int doSendMail(MailDTO pDTO) {
		...
		// 전달 받은 DTO로부터 데이터 가져오기(DTO객체가 메모리에 올라가지 않아 Null이 발생할 수 있기 때문에 에러방지차원에서 if문
		// 사용함)
		if (pDTO == null) {
			pDTO = new MailDTO();
		}
		...
		// 로그 찍기(추후에 찍은 로그를 통해서 이 함수 호출이 끝났는지 파악하기 용이하다.)
